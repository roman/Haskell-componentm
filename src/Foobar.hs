...
import           Control.Monad.Component (ComponentM, buildComponent,
                                          buildComponent_, fromComponent,
                                          getTeardown, runComponentM)
import           Control.Teardown        (renderTeardownReport, runTeardown)
...
compA = buildComponent "A" $ do
  ...
  tid        <- forkIO
    $ forever (atomically (readTBQueue inputQueue) >>= Prelude.print)
...
compC = buildComponent_ "C" $
  return "hello world"

compD :: ComponentM Text
compD = buildComponent_ "D" $
  return "Hola Mundo"

buildThreads :: Show a => ComponentM (a -> IO (), Int -> IO ())
buildThreads = (,) <$> compA <*> compB
