...
PROJECT_NAME ?= "componentm"
PROJECT_VERSION:=$(shell cat $(PROJECT_NAME)/package.yaml | grep -v '\#' | grep version | awk '{print $$2}' | sed -e "s;'\(.*\)';\1;")

STACK := stack --resolver nightly

SDIST_DIR_NAME:=$(PROJECT_NAME)-$(PROJECT_VERSION)
INTERNAL_SDIST_TAR:=$(PROJECT_NAME)/$$($(STACK) path --dist-dir)/$(SDIST_DIR_NAME).tar.gz
PROJECT_SDIST_TAR=out/$(SDIST_DIR_NAME).tar.gz

################################################################################

$(PROJECT_SDIST_TAR):
	$(STACK) sdist $(PROJECT_NAME) --pvp-bounds lower
